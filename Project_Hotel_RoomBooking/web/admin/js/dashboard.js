...
function updateChart(period) {
  ...
  const activeBtn = document.querySelector(
    `.btn-period[onclick="updateChart('${period}')"]`
  );
  if (activeBtn) {
...
function showNotification(message, type = "info") {
  ...
  notification.className = `alert alert-${
    type === "info" ? "primary" : type === "success" ? "success" : "danger"
...
function exportReport() {
  ...
  const occupancyRate =
    document.querySelector(".kpi-card h3").textContent || "0";
  const totalRooms =
    document.querySelector("[data-total-rooms]")?.textContent || "0";
  const occupiedRooms =
    document.querySelector("[data-occupied-rooms]")?.textContent || "0";
  const vacantRooms =
    document.querySelector("[data-vacant-rooms]")?.textContent || "0";
  const maintenanceRooms =
    document.querySelector("[data-maintenance-rooms]")?.textContent || "0";
  const totalRevenue =
    document.querySelector("[data-total-revenue]")?.textContent || "0";
  const averageRating =
    document.querySelector("[data-average-rating]")?.textContent || "0";
...
window.initRoomStatusWithServerData = function (
  occupiedRooms,
  vacantRooms,
  maintenanceRooms
) {
