...
// ✅ THÊM DEBUG
        ...
        console.log("Input name:", keywordInput.name);
        console.log("Input value:", keywordInput.value);
        ...
        // ✅ QUAN TRỌNG: Submit form sau khi validation thành công
        form.submit();
...
// Tự động ẩn và remove message sau 5 giây
setTimeout(() => {
    const errorDiv = document.getElementById("serverErrorMessage");
    if (errorDiv) {
        // Ẩn dần bằng CSS transition (nếu có)
        errorDiv.style.transition = "opacity 0.5s ease";
        errorDiv.style.opacity = 0;

        // Sau khi ẩn (0.5s), remove khỏi DOM
        setTimeout(() => {
            errorDiv.remove();
        }, 500);
    }
